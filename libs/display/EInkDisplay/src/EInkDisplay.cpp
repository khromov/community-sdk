...
// Ultra-fast 4-level grayscale LUT - conservative approach
// Based on working lut_grayscale with reduced timing
const unsigned char lut_ultrafast_4gray[] PROGMEM = {
    // VS waveforms (5 groups × 10 bytes = 50 bytes)
    // Simplified patterns - fewer active bytes per level

    // L0: No change (keep same as original)
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

    // L1: Light gray - reduced from 3 bytes to 2 bytes active
    0x54, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

    // L2: Medium gray - reduced from 3 bytes to 2 bytes active
    0xAA, 0xA8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

    // L3: Dark gray - reduced from 3 bytes to 2 bytes active
    0xA2, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

    // L4: VCOM (keep zeros)
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

    // TP/RP timing groups (10 groups × 5 bytes = 50 bytes)
    // Reduced from 3 active groups to 2
    0x01, 0x01, 0x01, 0x01, 0x00,  // G0: keep 4 frames
    0x01, 0x01, 0x01, 0x01, 0x00,  // G1: keep 4 frames
    0x00, 0x00, 0x00, 0x00, 0x00,  // G2: DISABLED (was active)
    ...
    // Frame rate - increased from 0x8F to 0xCF
    0xCF, 0xCF, 0xCF, 0xCF, 0xCF,

    // Voltages - IDENTICAL to working LUT
    0x17, 0x41, 0xA8, 0x32, 0x30,
...
// Corresponding revert LUT
const unsigned char lut_ultrafast_4gray_revert[] PROGMEM = {
    // VS waveforms - simplified cleanup patterns
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // L0
    0x54, 0x54, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // L1
    0xA8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // L2
    0xFC, 0xFC, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // L3
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // VCOM

    // TP/RP - 2 active groups (matches forward LUT)
...
void EInkDisplay::refreshDisplay(const RefreshMode mode, const bool turnOffScreen) {
  ...
  if (mode == FULL_REFRESH) {
    ...
    sendData(0x5A);
    displayMode |= 0xD4;
  } else if (mode == ULTRA_FAST_REFRESH) {
    // Use ultra-fast custom LUT
    setCustomLUT(true, lut_ultrafast_4gray);
    displayMode |= 0x0C;  // MODE_SELECT | DISPLAY_START (custom LUT active)
  } else {  // FAST_REFRESH
