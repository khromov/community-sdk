...
// Ultrafast LUT: EXACT copy of lut_grayscale with only frame rate increased
const unsigned char lut_ultrafast_4gray[] PROGMEM = {
    ...
    // TP/RP groups (global timing) - reduced to 2 groups (8 frames vs 12)
    0x01, 0x01, 0x01, 0x01, 0x00,  // G0: A=1 B=1 C=1 D=1 RP=0 (4 frames)
    0x01, 0x01, 0x01, 0x01, 0x00,  // G1: A=1 B=1 C=1 D=1 RP=0 (4 frames)
    ...
    // Frame rate - increased to 0xBF for faster refresh
    0xBF, 0xBF, 0xBF, 0xBF, 0xBF,

    // Voltages (VGH, VSH1, VSH2, VSL, VCOM) - same as lut_grayscale
    0x17, 0x41, 0xA8, 0x32, 0x30,

    // Reserved
    0x00, 0x00};

// Ultrafast revert LUT: EXACT copy of lut_grayscale_revert with only frame rate increased
const unsigned char lut_ultrafast_4gray_revert[] PROGMEM = {
    ...
    // TP/RP groups (global timing) - reduced to 2 groups (8 frames vs 16)
    0x01, 0x01, 0x01, 0x01, 0x01,  // G0: A=1 B=1 C=1 D=1 RP=0 (4 frames)
    0x01, 0x01, 0x01, 0x01, 0x01,  // G1: A=1 B=1 C=1 D=1 RP=0 (4 frames)
    0x00, 0x00, 0x00, 0x00, 0x00,  // G2: DISABLED
    0x00, 0x00, 0x00, 0x00, 0x00,  // G3: DISABLED
    ...
    // Voltages (VGH, VSH1, VSH2, VSL, VCOM) - same as lut_grayscale_revert
    0x17, 0x41, 0xA8, 0x32, 0x30,

    // Reserved
    0x00, 0x00};

// BACKUP: Working hybrid LUT (LUT #2 timing) - kept for reference
// This version worked without flickering: original waveforms + fast Chinese timing
const unsigned char lut_ultrafast_bw_working_tmp[] PROGMEM = {
    // L0-L4: Original B/W waveforms
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    // Timing: Chinese LUT #2 (fast)
    0x02, 0x01, 0x02, 0x01, 0x04,  // G0
    0x04, 0x00, 0x04, 0x04, 0x04,  // G1
    0x04, 0x00, 0x06, 0x01, 0x03,  // G2
    0x01, 0x01, 0x02, 0x01, 0x01,  // G3
    0x00, 0x00, 0x00, 0x00, 0x00,  // G4-G9 disabled
    ...
    0x00, 0x00, 0x00, 0x00, 0x00,
    // Frame rate & voltages
    0x55, 0x55, 0x55, 0x33, 0x33,
    0x17, 0x41, 0xA8, 0x28, 0x30,
    0x00, 0x00};

// Hybrid LUT: Original B/W waveforms + Chinese LUT #1 timing (quality mode)
// Testing slower, more thorough timing from Chinese firmware
const unsigned char lut_ultrafast_bw[] PROGMEM = {
    // L0: No change (both black) - RED=0, BW=0
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    // Timing Groups - Chinese LUT #1 (quality - slower, more thorough)
    0x03, 0x07, 0x01, 0x1e, 0x00,  // G0: longer timing (3,7,1,30,0)
    0x03, 0x01, 0x03, 0x02, 0x05,  // G1: (3,1,3,2,5)
    0x05, 0x00, 0x05, 0x05, 0x05,  // G2: (5,0,5,5,5)
    0x05, 0x00, 0x08, 0x02, 0x04,  // G3: (5,0,8,2,4)
    0x01, 0x02, 0x03, 0x02, 0x02,  // G4: (1,2,3,2,2) - LUT #1 has this active!
    0x00, 0x00, 0x00, 0x00, 0x00,  // G5: Disabled
    0x00, 0x00, 0x00, 0x00, 0x00,  // G6: Disabled
    0x00, 0x00, 0x00, 0x00, 0x00,  // G7: Disabled
    0x00, 0x00, 0x00, 0x00, 0x00,  // G8: Disabled
    0x00, 0x01, 0x00, 0x00, 0x00,  // G9: Minimal active
    // Frame rate - LUT #1 values (slower = higher quality)
    0x44, 0x44, 0x44, 0x22, 0x22,
    // Voltages - Original (tuned for display)
    0x17, 0x41, 0xA8, 0x28, 0x30,
    // Reserved
    0x00, 0x00};
...
void EInkDisplay::grayscaleRevert() {
  ...
  // Load the revert LUT
  setCustomLUT(true, lut_ultrafast_4gray_revert);
...
void EInkDisplay::displayBuffer(RefreshMode mode) {
  ...
  // For fast refresh, use B/W-specific ultrafast LUT
  if (mode == FAST_REFRESH) {
    setCustomLUT(true, lut_ultrafast_bw);
  }
